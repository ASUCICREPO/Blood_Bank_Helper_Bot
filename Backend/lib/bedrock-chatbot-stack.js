...
class BedrockChatbotStack extends cdk.Stack {
  constructor(scope, id, props) {
    ...
    const guardrail = new bedrock.CfnGuardrail(this, "ChatGuardrail", {
      ...
      description:
        "Guardrail for the blood donation assistant: prompt-attack defense, harmful content filtering, PII protection, and topic restriction.",
    ...
    const chatLambdaRole = new iam.Role(this, "ChatLambdaRole", {
      ...
      description:
        "Role for Chat Lambda function to access Bedrock, S3, and DynamoDB",
      managedPolicies: [
    ...
    const knowledgeBaseRole = new iam.Role(this, "KnowledgeBaseRole", {
      ...
      description:
        "Role for Bedrock Knowledge Base to access S3 and OpenSearch - Updated for Data Automation",
    ...
    const vectorCollection =
      new generative_ai_cdk_constructs_1.opensearchserverless.VectorCollection(
        this,
        "BloodCentersVectorCollection",
        {
          description: `Vector collection for ${projectName} Knowledge Base`,
          standbyReplicas:
            generative_ai_cdk_constructs_1.opensearchserverless
              .VectorCollectionStandbyReplicas.DISABLED, // Cost optimization for dev
        },
      );
    // Create Vector Index within the OpenSearch Serverless collection
    const vectorIndex =
      new generative_ai_cdk_constructs_1.opensearch_vectorindex.VectorIndex(
        this,
        "BloodCentersVectorIndex",
        {
          collection: vectorCollection,
          indexName: cdk.Names.uniqueResourceName(this, {
            maxLength: 63,
            separator: "-",
          }).toLowerCase(),
    ...
    const defaultPolicyConstruct =
      knowledgeBaseRole.node.tryFindChild("DefaultPolicy");
    ...
    const webCrawlerDataSource =
      websiteSeedUrls.length > 0
    ...
    const dailySyncDataSource =
      dailySyncSeedUrls.length > 0
    ...
    const syncOperationsLambdaRole = new iam.Role(
      this,
      "SyncOperationsLambdaRole",
      {
        assumedBy: new iam.ServicePrincipal("lambda.amazonaws.com"),
        managedPolicies: [
          iam.ManagedPolicy.fromAwsManagedPolicyName(
    ...
    const syncOperationsLambda = new lambda.Function(
      this,
      "SyncOperationsLambdaFunction",
      {
    ...
    const startPdfSync = new stepfunctionsTasks.LambdaInvoke(
      this,
      "StartPdfSync",
      {
    ...
    const checkPdfStatus = new stepfunctionsTasks.LambdaInvoke(
      this,
      "CheckPdfStatus",
      {
        lambdaFunction: syncOperationsLambda,
    ...
    const startDailySync = new stepfunctionsTasks.LambdaInvoke(
      this,
      "StartDailySync",
      {
        lambdaFunction: syncOperationsLambda,
    ...
    const checkDailyStatus = new stepfunctionsTasks.LambdaInvoke(
      this,
      "CheckDailyStatus",
      {
        lambdaFunction: syncOperationsLambda,
    ...
    const startWebsiteSync = new stepfunctionsTasks.LambdaInvoke(
      this,
      "StartWebsiteSync",
      {
        lambdaFunction: syncOperationsLambda,
    ...
    const definition = startPdfSync
      ...
      .next(checkPdfStatus)
      .next(
        new stepfunctions.Choice(this, "IsPdfComplete?")
          .when(
            stepfunctions.Condition.booleanEquals(
              "$.pdfStatus.Payload.isComplete",
              true,
            ),
            new stepfunctions.Choice(this, "IsPdfSuccess?")
              .when(
                stepfunctions.Condition.booleanEquals(
                  "$.pdfStatus.Payload.isSuccess",
                  true,
                ),
                startDailySync
                  .next(waitForDaily)
                  ...
                  .next(
                    new stepfunctions.Choice(this, "IsDailyComplete?")
                      .when(
                        stepfunctions.Condition.booleanEquals(
                          "$.dailyStatus.Payload.isComplete",
                          true,
                        ),
                        new stepfunctions.Choice(this, "IsDailySuccess?")
                          .when(
                            stepfunctions.Condition.booleanEquals(
                              "$.dailyStatus.Payload.isSuccess",
                              true,
                            ),
    ...
    const sequentialSyncStateMachine = new stepfunctions.StateMachine(
      this,
      "SequentialSyncStateMachine",
      {
    ...
    const dailySyncLambda = new lambda.Function(
      this,
      "DailySyncLambdaFunction",
      {
        runtime: lambda.Runtime.PYTHON_3_11,
        ...
        description:
          "Daily Sync Automation for Blood Centers Daily Data Source",
      },
    );
    ...
    const dailySyncRule = new events.Rule(this, "DailySyncRule", {
      ...
      description:
        "Triggers daily sync of blood centers daily data source at 2 PM EST (7 PM UTC)",
    ...
    dailySyncRule.addTarget(
      new targets.LambdaFunction(dailySyncLambda, {
    ...
    new cdk.CfnOutput(this, "ChatLambdaUrl", {
      value: chatLambdaUrl.url,
      description:
        "Lambda Function URL for streaming chat (use for both REACT_APP_CHAT_ENDPOINT and REACT_APP_API_BASE_URL)",
